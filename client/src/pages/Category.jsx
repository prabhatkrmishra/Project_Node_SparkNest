...
const Category = () => {
  ...
  const [displayCategory, setDisplayCategory] = useState(
    sessionStorage.getItem("previousCategory") || "Design"
  );

  useEffect(() => {
    const previousCategory =
      sessionStorage.getItem("previousCategory") || categoryType;
    if (categoryType !== previousCategory) {
      sessionStorage.setItem("previousCategory", categoryType);
      window.location.reload();
    } else {
      setDisplayCategory(
        previousCategory.charAt(0).toUpperCase() + previousCategory.slice(1)
      );
    }
  }, [categoryType]);
  ...
  return (
    <div>
      <Helmet>
        <title>BloggingVerse : Category - {displayCategory}</title>
      </Helmet>
      <PreLoader/>
