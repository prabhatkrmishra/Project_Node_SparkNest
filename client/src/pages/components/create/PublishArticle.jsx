...
import { fetchCategories, createArticle } from "../../../api/API";
...
import { getItem, removeItem } from "../tools/IndexDBstorage";
...
const PublishArticle = () => {
  ...
  const getBase64String = async (imageBlob) => {
    try {
  ...
  useEffect(() => {
    setTitle(trimTitle(localStorage.getItem("articleTitle")));
    setInfo(trimInfo(data.body_text));

    const loadContent = async () => {
      body.current = await getItem("articleContent");
    };
    loadContent();
  }, [data]);
